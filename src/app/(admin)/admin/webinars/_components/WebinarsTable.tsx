        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            ...
            {webinars.map(w => {
              ...
              return (
                <tr key={w.id} className="hover:bg-[rgba(255,255,255,0.04)]" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                  <td style={{ padding: '11px 14px', maxWidth: 260 }}>
                    <p style={{ fontSize: 13, fontWeight: 600, color: '#f8fafc', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{w.title}</p>
                  </td>
                  <td style={{ padding: '11px 14px', fontSize: 13, color: '#94a3b8' }}>{w.presenterName ?? '—'}</td>
                  <td style={{ padding: '11px 14px', fontSize: 13, color: '#94a3b8' }}>{w.platform ?? '—'}</td>
