      ...
      <div className={`bg-white rounded-[10px] border border-[#e2e8f0] overflow-hidden transition-opacity ${isPending ? 'opacity-60' : ''}`}>
        <table className="w-full border-collapse">
          ...
          <tbody>
            ...
            {programs.map(prog => (
              <tr key={prog.id} className="border-t border-[#f1f5f9] hover:bg-[#f8fafc] transition-colors">
                <td className="px-3.5 py-[11px] text-[13px] text-[#334155]">
                  <strong className="text-[#0f172a] font-semibold">{prog.title}</strong>
                ...
                <td className="px-3.5 py-[11px]">
                  <div className="flex flex-wrap gap-1">
                    {prog.stacks.map(({ stack }) => (
                      <Badge key={stack.id} variant="light">{stack.name}</Badge>
                    ))}
                  </div>
                </td>
                <td className="px-3.5 py-[11px] text-[13px] text-[#334155] whitespace-nowrap">{prog.duration ?? '—'}</td>
